import { getGoogleReviews, getOptions, getSinglePostData } from '@/utils/fetchData'
...
export async function generateMetadata({ params, searchParams }, parent) {
  ...
  // fetch data
  const data = await getSinglePostData("event-buffet", "/wp-json/wp/v2/pages")
  ...
  if (data.length > 0) {
    ...
    return {
      ...
      metadataBase: new URL(process.env.siteUrl),
         alternates: {
          canonical: `${process.env.siteUrl}/event-buffet`, 
        },
      openGraph: {
        title: seoData?.title,
        description: seoData?.description,
...
export default async function Page() {
  ...
  const postData = await getSinglePostData("event-buffet", "/wp-json/wp/v2/pages")
  ...
  return (
    <>
      ...
      <Footer copyrightMarketStorey={true} tripAdvisorData={options.trip_advisor} lightCTA={true} footerCtaData={options.footer_cta} certifications={options.certifications} contactInfo={options.contact_info} socialData={options.social_links} />
    </>
